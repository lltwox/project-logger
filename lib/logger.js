...
var Loggers = {
  ...
  cluster: require('./transports/cluster'),
...
Logger.prototype.configure = function(config) {
  ...
  if (config.format) this.updateFormat();
};
 ...
 * @private
 */
Logger.prototype.updateFormat = function() {
  var format = this.config.format,
      parts = [];

  Object.keys(FORMAT_PLACEHOLDERS).forEach(function(placeholder) {
    var index = format.indexOf('{' + placeholder + '}');
    if (index >= 0) {
      format = format.replace(
        '\{' + placeholder + '\}', FORMAT_PLACEHOLDERS[placeholder]
      );
      parts.push({
        part: placeholder,
        index: index
  ...
  this.format = format;
  parts = _.sortBy(parts, 'index');

  this.getFormatParams = function() {
    var result = [];
    for (var i = 0; i < parts.length; i++) {
      result.push(FORMAT_PARAMS[parts[i].part].apply(this, arguments));
    }
    return result;
  };
};
...
Logger.prototype.getMessagePrefix = function(level) {
  return util.format.apply(
    util, [this.format].concat(this.getFormatParams(level))
  );
};
